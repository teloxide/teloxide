...
mod generated {
    use crate::dispatching::{core::IntoGuard, dispatcher_context::DispatcherContext};
    ...
    ($(($ident:ident, $item:ty, $get_field:expr),)*) => {$(const _: () = {
        ...
        impl<G: Guard<$item>> Guard<DispatcherContext<Message>> for Checker<G> {
            fn check<'a>(&self, cx: &'a DispatcherContext<Message>) -> BoxFuture<'a, bool> {
        ...
        impl<G: Guard<$item>> IntoGuard<DispatcherContext<Message>, Checker<G>> for Checker<G> {
